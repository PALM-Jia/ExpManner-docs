...
本网站提供 ExpManner 的公开中文说明。代码仓库 `PALM-Jia/ExpManner` 是 private repo，需要 PALM Jia 授权访问。
...
| 数据集维护者 | 先读 [数据集指南](dataset-guide.md)，再看 [集成聚类](tutorials/ensemble-clustering.md) |
...
5. 使用 `Models.KMeans()` 跑第一个 benchmark。
...
公开说明只写安全内容。可以公开接口、toy dataset 示例、结果目录格式和贡献流程；不要公开私有数据路径、凭据、密钥、未发表实验结果、内部 benchmark 表格或项目侧敏感模型实现。
